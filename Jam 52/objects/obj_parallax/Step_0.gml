if (!obj_torch.dreamMode) {
	...
	_cx += _xspd
	camera_set_view_pos(view_camera[0], _cx, obj_camera.y-540);
	...
	var _b = ds_map_find_first(background_map2);
	repeat(ds_map_size(background_map2))
		{
		layer_x(_b, background_map2[? _b] * _cx);
		_b = ds_map_find_next(background_map2, _b);
		}
}
...
if (instance_exists(obj_torch)) {
	if (obj_torch.dreamMode) {
		...
		layer_set_visible("D_Tiles2",true);
	}
	else {
		...
		layer_set_visible("D_Tiles2",false);
	}
}
